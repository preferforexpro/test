...
@app.get("/")
async def root():
    # This path must point to where your index.html is stored
    # If you moved frontend inside the backend folder, use this:
    index_path = os.path.join(os.path.dirname(__file__), "frontend", "index.html")
    
    if os.path.exists(index_path):
        return FileResponse(index_path)
    
    # If it fails, this will tell you exactly where the server is looking
    return {
        "error": "Dashboard not found",
        "looking_at_path": index_path,
